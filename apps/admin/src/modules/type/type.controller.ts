import { Controller, Post, Delete, Put, Param, ForbiddenException, Body, Get, BadRequestException, InternalServerErrorException } from "@nestjs/common";
...
export class TypeController {
  constructor(private readonly typeService: TypeService, private readonly verificationService: VerifyService, @InjectModel(Type.name) private readonly model: Model<Type>) {}
  @Roles("admin")
  @Get("findType1/:mid")
  ...
  async create(@Body() doc: Type) {
    ...
    const findNameRes = await this.verificationService.testOneExist(Type.name, "type_name", type_name);
    const findPIdRes = !!type_pid ? await this.verificationService.testOneExist(Type.name, "_id", type_pid) : false;
    const findGroupRes = await this.verificationService.testAllExist(Group.name, "_id", group_ids);
    // 检查分类名是否存在
  ...
  @Roles("admin")
  @Put("update/:id")
  async update(@Body() doc: Type, @Param(new VerifyDtoPipe("ObjectId", "id", null)) id: string) {
    const { group_ids, type_pid, type_name, type_mid } = doc;
    const findNameRes = await this.verificationService.testOneExist(Type.name, "type_name", type_name);
    const findSubTypeRes = await this.verificationService.testOneExist(Type.name, "type_pid", id);
    const findPIdRes = !!type_pid ? await this.verificationService.testOneExist(Type.name, "_id", type_pid) : false;
    const findIdRes = await this.verificationService.testOneExist(Type.name, "_id", id);
    const findGroupRes = await this.verificationService.testAllExist(Group.name, "_id", group_ids);
    // 检查分类名是否存在
    ...
    // 检查是否需要更新
    if (Object.keys(doc).every((item) => doc[item].toString() === findIdRes[item].toString())) {
  ...
  @Roles("admin")
  @Put("changStatus/:id")
  async changStatus(@Param(new VerifyDtoPipe("ObjectId", "id", null)) id: string, @Body(new VerifyDtoPipe("document", "self", null)) body) {
    const findIdRes = await this.verificationService.testOneExist(Type.name, "_id", id);
    // 如果不存在
    if (!findIdRes) {
  ...
  @Roles("admin")
  @Delete("deleteMany")
  async deleteMany(@Body(new VerifyDtoPipe("ObjectIdArray", "self", null)) _idArr: string[]) {
    const findSubTypeRes = await this.verificationService.testInOneExists(Type.name, "type_pid", _idArr);
    if (findSubTypeRes) {
      throw new ForbiddenException("请先清理子分类");
    }
    return this.model
